...
>**警告**
>
>需要指出，某些教程建议用户关闭 ACPI，这种做法目前已缺乏依据。除非你不是 x86，否则对于现代计算机绝没有关闭的道理。ACPI 与电源状态管理、设备节能、多处理器支持等功能密切相关。应将该选项视为遗留功能，仅在不支持 UEFI 的旧式计算机上才需要考虑。
>
...
此为键盘布局菜单，直接按 **回车键** 使用默认的美式（US）键盘布局即可（因目前中国使用美式键盘布局）。
...
>**警告**
>
>官方手册中的说法（关于 `Amnesiac`）有误。若不设置主机名，系统不会自动分配任何值（包括 `Amnesiac`）。因为 FreeBSD 源码默认假设将通过 DHCP 获取主机名。根据当前源码逻辑，使用 DHCP 时不会有空主机名提示；仅当无网络连接时，登录信息中才会显示 `Amnesiac` 并伴随一条错误信息。
...
>**技巧**
>
>经测试，在 UEFI 环境下，256MB 内存亦可运行 ZFS；若使用传统的 BIOS，128MB 内存即足够。
...
此为最终警告与确认。请确保已备份重要数据，所选磁盘将被完全格式化。使用 **方向键** 或 **Tab 键** 将焦点切换至 `<YES>`，按 **回车键** 确认。
...
（假设根分区为 `/dev/nda0p2`）执行 ：
若需要在 LiveCD 环境中挂载该加密分区，操作亦较为简单（不需要密钥）。执行以下命令：
（假设根分区为 `/dev/nda0p2`）执行 ：
```sh
...
CST 为中国标准时间（China Standard Time）的缩写，无误，按 **回车键** 选择 `Yes`。
...
自动检测并安装所需硬件固件（此功能自 14.2 版本[引入](https://cgit.freebsd.org/src/commit/?id=03c07bdc8b31)）。
...
>**技巧**
>
>FreeBSD 基本系统默认不包含图形界面（未安装 Xorg），因此启动后为文本控制台界面。
