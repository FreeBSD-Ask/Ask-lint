...
主要面向 x86-64（amd64）、AArch64（arm64），并尽可能多地支持其他硬件架构。
...
涉及一些 Unix 基础知识和 FreeBSD 特色功能。主要面向 Windows 用户和 Linux 用户。即使你认为自己已经掌握了一些，也建议概览一下本章各子目录。
...
尽可能地罗列了所有可在 FreeBSD 上运行的桌面环境。如果没有你喜欢的桌面环境，欢迎 PR。
...
使用手动及其他专业安装方式和介质，在不同平台上安装 FreeBSD。
...
将会不断地弹出来窗口询问你怎么选。如果是使用默认选项，请这样做：
